...
import Windek from '../windek/windek.component';
import './windek-directory.styles.css'

class Windekdirectory extends React.Component {
    constructor() {
        super();
        this.state = {
          sections: [{
    ...
    render() {
        return(
        <div className='windek-directory'>
            {
                this.state.sections.map(({title, imageUrl, id}) => (
                    <Windek key={id} title={title} imageUrl={imageUrl} />
                ))}
        </div>
        )
    }
}
export default Windekdirectory